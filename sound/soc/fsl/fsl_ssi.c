...
#ifdef SSI_DEBUG
void ssi_writel2(u32 b, volatile void __iomem *addr) {
        printk(KERN_WARNING "fsl_ssi.c: value 0x%x was written into the address 0x%x\n", b, (unsigned long)addr & 0x1FFF);
        writel(b, addr);
}
#else
void ssi_writel2(u32 b, volatile void __iomem *addr) {
...
#define write_ssi(val, addr)		 ssi_writel2(val, addr)
/*
...
static inline void write_ssi_mask(u32 __iomem *addr, u32 clear, u32 set)
{
	...
	val = (val & ~clear) | set;
/*#ifdef SSI_DEBUG
	printk(KERN_WARNING "fsl_ssi.c: using original mask 0x%x and modified mask 0x%x\n", clear, val);
#endif*/
	writel(val, addr);
}
#endif
...
{
	printk(KERN_WARNING "fsl_ssi.c:------------%s was called\n", __func__);
	switch (reg) {
	case 0x0:	/* stx0 */
	case 0x4:	/* stx1 */
	case 0x8:	/* srx0 */
	...
	struct snd_pcm_hw_params *hw_params, struct snd_soc_dai *cpu_dai)
{
	...
	if (ssi_private->i2s_mode == CCSR_SSI_SCR_I2S_MODE_MASTER) {
#ifdef SSI_DEBUG
		printk(KERN_WARNING "fsl_ssi.c:------------%s ssi_private->i2s_mode == CCSR_SSI_SCR_I2S_MODE_MASTER\n", __func__);
#endif
		ret = fsl_ssi_set_bclk(substream, cpu_dai, hw_params);
		if (ret) {
#ifdef SSI_DEBUG
			printk(KERN_WARNING "fsl_ssi.c:------------%s fsl_ssi_set_bclk return ERROR\n", __func__);
#endif
			return ret;
		}
		...
		if (!(ssi_private->baudclk_streams & BIT(substream->stream))) {
			...
			if (ret) {
#ifdef SSI_DEBUG
				printk(KERN_WARNING "fsl_ssi.c:------------%s clk_prepare_enable return ERROR\n", __func__);
#endif
				return ret;
			}
			   ...
			   struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			write_ssi_mask(&ssi->scr, 0, CCSR_SSI_SCR_SSIEN | CCSR_SSI_SCR_TE);
			write_ssi_mask(&ssi->sier, 0, CCSR_SSI_SIER_TDMAE);
		} else {
			write_ssi_mask(&ssi->scr, 0, CCSR_SSI_SCR_SSIEN | CCSR_SSI_SCR_RE);
...
static int fsl_ssi_probe(struct platform_device *pdev)
{
	...
	ssi_private = kzalloc(sizeof(struct fsl_ssi_private) + strlen(p), GFP_KERNEL);
	if (!ssi_private) {
	...
	memcpy(&ssi_private->cpu_dai_drv, &fsl_ssi_dai_template, sizeof(fsl_ssi_dai_template));
